...
var LemonSqueezy = class {
  ...
  async getStore({ id, ...params } = {}) {
    if (!id)
      throw "You must provide an ID in getStore().";
    params = this.buildParams(params);
  ...
  async getProduct({ id, ...params } = {}) {
    if (!id)
      throw "You must provide an ID in getProduct().";
    params = this.buildParams(params);
  ...
  async getVariant({ id, ...params } = {}) {
    if (!id)
      throw "You must provide an ID in getVariant().";
    params = this.buildParams(params);
  ...
  async getCheckout({ id, ...params } = {}) {
    if (!id)
      throw "You must provide an ID in getCheckout().";
    params = this.buildParams(params);
  ...
  async createCheckout({ storeId, variantId, attributes = {} } = {}) {
    if (!storeId)
      throw "You must provide a store ID in createCheckout().";
    if (!variantId)
      throw "You must provide a variant ID in createCheckout().";
    let payload = {
      "data": {
        "type": "checkouts",
  ...
  async getCustomer({ id, ...params } = {}) {
    if (!id)
      throw "You must provide an ID in getCustomer().";
    params = this.buildParams(params);
  ...
  async getOrder({ id, ...params } = {}) {
    if (!id)
      throw "You must provide an ID in getOrder().";
    params = this.buildParams(params);
  ...
  async getFile({ id, ...params } = {}) {
    if (!id)
      throw "You must provide an ID in getFile().";
    params = this.buildParams(params);
  ...
  async getOrderItem({ id, ...params } = {}) {
    if (!id)
      throw "You must provide an ID in getOrderItem().";
    params = this.buildParams(params);
  ...
  async getSubscription({ id, ...params } = {}) {
    if (!id)
      throw "You must provide an ID in getSubscription().";
    params = this.buildParams(params);
  ...
  async updateSubscription({ id, variantId, productId, billingAnchor, proration } = {}) {
    if (!id)
      throw "You must provide an ID in updateSubscription().";
    let attributes = {
  ...
  async cancelSubscription({ id }) {
    if (!id)
      throw "You must provide an ID in cancelSubscription().";
  ...
  async resumeSubscription({ id }) {
    if (!id)
      throw "You must provide an ID in resumeSubscription().";
    let payload = {
      data: {
        type: "subscriptions",
        id: "" + id,
  ...
  async pauseSubscription({ id, mode, resumesAt } = {}) {
    if (!id)
      throw "You must provide an ID in pauseSubscription().";
    let pause = { mode: "void" };
    if (mode)
      pause.mode = mode;
    if (resumesAt)
  ...
  async unpauseSubscription({ id }) {
    if (!id)
      throw "You must provide an ID in unpauseSubscription().";
    let payload = {
      data: {
        type: "subscriptions",
        id: "" + id,
  ...
  async getSubscriptionInvoice({ id, ...params } = {}) {
    if (!id)
      throw "You must provide an ID in getSubscriptionInvoice().";
    params = this.buildParams(params);
  ...
  async getDiscount({ id, ...params } = {}) {
    if (!id)
      throw "You must provide an ID in getDiscount().";
    params = this.buildParams(params);
  ...
  async createDiscount({
    ...
    if (!storeId)
      throw "You must include a `storeId` in createDiscount().";
    if (!name)
      throw "You must include a `name` in createDiscount().";
    if (!code)
      throw "You must include a `code` in createDiscount().";
    if (!amount)
      throw "You must include an `amount` in createDiscount().";
    let attributes = {
      name,
      code,
      amount,
  ...
  async deleteDiscount({ id }) {
    if (!id)
      throw "You must provide an ID in deleteDiscount().";
  ...
  async getDiscountRedemption({ id, ...params } = {}) {
    if (!id)
      throw "You must provide an ID in getDiscountRedemption().";
    params = this.buildParams(params);
  ...
  async getLicenseKey({ id, ...params } = {}) {
    if (!id)
      throw "You must provide an ID in getLicenseKey().";
    params = this.buildParams(params);
  ...
  async getLicenseKeyInstance({ id, ...params } = {}) {
    if (!id)
      throw "You must provide an ID in getLicenseKeyInstance().";
    params = this.buildParams(params);
  ...
  async getWebhook({ id, ...params } = {}) {
    if (!id)
      throw "You must provide an ID in getWebhook().";
    params = this.buildParams(params);
  ...
  async createWebhook({ storeId, url, events, secret } = {}) {
    if (!storeId)
      throw "You must provide a store ID in createWebhook().";
    if (!url)
      throw "You must provide a URL in createWebhook().";
    if (!events || events?.length < 1)
      throw "You must provide a list of events in createWebhook().";
    if (!secret)
      throw "You must provide a signing secret in createWebhook().";
    let payload = {
      data: {
        type: "webhooks",
        attributes: {
  ...
  async updateWebhook({ id, url, events, secret } = {}) {
    if (!id)
      throw "You must provide an ID in updateWebhook().";
    let attributes = {};
    if (url)
      attributes.url = url;
    if (events)
  ...
  async deleteWebhook({ id }) {
    if (!id)
      throw "You must provide an ID in deleteWebhook().";
    this.queryApi({ path: "v1/webhooks/" + id, method: "DELETE" });
  }
};
export {
  LemonSqueezy as default
};
